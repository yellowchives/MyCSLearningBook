...
数组的长度有限，对长度取模运算一定会有很多key计算出来的索引一样，这种情况叫做**哈希冲突**。 哈希冲突是无法避免的，因为求hash是一种压缩映射，值空间远小于输入的空间。解决哈希冲突的方法主要有两种，一种是开放寻址法，一种是链表法。开放寻址法的原理很简单，当一个Key通过哈希函数获得对应的数组下标已被占用时，就向后寻找下一个空档位置。在Java中，ThreadLocal所使用的就是开放寻址法。另一种方法是链表法，就是把下标相同的Entry串成一个链表，HashMap就采用这种方式。
...
一个好的散列函数通常倾向于“为不相等的对象产生不相等的散列码”。理想情况下，散列函数应该把集合中不相等的实例均匀地分布到所有可能的散列值上。要想完全达到这种理想的情形是非常困难的。幸运的是，相对接近这种理想情形则并不太困难。Times 33算法，它是有史以来发布的最有效的哈希函数之一。

核心逻辑是这段代码：`hash(i) = hash(i-1) * 33 + str[i];`
神奇数字 33 是通过不断测试得到的。经过测试发现偶数的均匀效果很差，而 33 效果很好，而且接近 32，可以通过以为来快速计算。
...
```java
   public class Time33 {
     ...
     * 乘法运算版本
     */
    public static int time33V1(String string) {
        int hash = 0;
        for (int i = 0; i < string.length(); i++) {
            hash = 33 * hash + string.charAt(i);
        }
        return hash;
    }
     ...
     * 位运算版本
     */
    public static int time33V2(String string) {
        int hash = 0;
        for (int i = 0; i < string.length(); i++) {
     ...
     * 5381版本
     */
    public static int time33V3(String string) {
        int hash = 5381;
        for (int i = 0; i < string.length(); i++) {
            hash = ((hash << 5) + hash) + string.charAt(i);
        }
...
```
Time33虽然可以较好地解决冲突，但是有可能相似字符串的HashCode也比较接近，如字符串“AA”的HashCode是2210，字符串“AB”的HashCode是2211。这在某些应用场景是不能接受的。一个可行的方案是对字符串取消息摘要，再对摘要求HashCode，由于字符串微小的变化就可以引起信息指纹的巨大不同，因此可以获得较好的随机散列。`字符串 -> MD5 -> Time33`
